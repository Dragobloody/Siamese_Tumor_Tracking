...
from MaskRCNN import utils, resnet, rpn, proposal
...
def postprocess_max_mask(masks, centers, config):
    ...
    if config.CROP:
        k = 4
    pad_t = tf.cast(centers[0, 0] - config.KERNEL_IMAGE_SHAPE[0]//k, 'int32')
    pad_l = tf.cast(centers[0, 1] - config.KERNEL_IMAGE_SHAPE[1]//k, 'int32')
    pad_b = tf.cast(config.SEARCH_IMAGE_SHAPE[0] - (centers[0, 0] + config.KERNEL_IMAGE_SHAPE[0]//k), 'int32')
    pad_r = tf.cast(config.SEARCH_IMAGE_SHAPE[1] - (centers[0, 1] + config.KERNEL_IMAGE_SHAPE[1]//k), 'int32')
    
    full_masks = tf.pad(masks[0], [[pad_t, pad_b], [pad_l, pad_r], [0, 0]])    
    full_masks = tf.expand_dims(full_masks, 0)
...
class AnchorlessDetectionLayerMask(KE.Layer):
    ...
    def call(self, inputs):
        ...
        if self.config.MASK:
            ...
            #start = time.time()
            #pred_masks = postprocess_mask(pred_masks, bbox_centers, self.config)
            #stop = time.time()
            #print(stop - start)
        # Apply scale/ratio penalty
        kernel_bbox = input_image_metas[:, 11:]
        ...
        if self.config.MASK:
            ...
            max_pred_mask = postprocess_max_mask(max_pred_mask, max_bbox_centers, self.config)
    ...
    def compute_output_shape(self, input_shape):
         return [           
            (self.config.BATCH_SIZE, None, 4),           
            (self.config.BATCH_SIZE, None, 4),  
            (self.config.BATCH_SIZE, None, None, None, 1), 
            (self.config.BATCH_SIZE, 8),        
            (self.config.BATCH_SIZE, None, None, 1), 
            (self.config.BATCH_SIZE, None, 1)
            ] 
