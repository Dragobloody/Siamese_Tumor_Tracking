...
def mIOU(gt_boxes, pred_boxes):
    overlaps = np.diagonal(utils.compute_overlaps(np.squeeze(pred_boxes, axis = 1), np.squeeze(gt_boxes, axis = 1)))
        
    return np.mean(overlaps)


def accuracy(gt_boxes, pred_boxes, iou_threshold = 0.7):
    overlaps = np.diagonal(utils.compute_overlaps(np.squeeze(pred_boxes, axis = 1), np.squeeze(gt_boxes, axis = 1)))
    positive = np.sum(overlaps >= iou_threshold)
...
def concordance_correlation_coefficient(gt_boxes, pred_boxes):   
