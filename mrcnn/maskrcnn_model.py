...
from MaskRCNN import utils, resnet, rpn as rp, proposal, detection, fpn, losses, data_generator as dg
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # RPN Model
        rpn = rp.build_rpn_model(config.RPN_ANCHOR_STRIDE,
        ...
        if mode == "training":
            ...
            rois, target_class_ids, target_bbox, target_mask =\
                detection.DetectionTargetLayer(config, name="proposal_targets")([
            ...
            # Model
            inputs_2d = [input_image, input_image_meta,
                      input_rpn_match, input_rpn_bbox, input_gt_class_ids, input_gt_boxes, input_gt_masks]
           
            if config.MASK:
                outputs_2d = [gt_boxes, rpn_class_logits, rpn_class, rpn_bbox,
                           mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask,
                           rpn_rois, output_rois, target_class_ids, target_mask,
            ...
            model = KM.Model(inputs_2d, outputs_2d, name='mask_rcnn')
            ...
            detections = detection.DetectionLayer(config, name="mrcnn_detection")(
        ...
        outputs_2d = [P2, P3, P4,
                      ...
                      rpn_rois, anchors, rois, target_class_ids, target_bbox, target_mask
                      ]
        model = KM.Model(inputs_2d, outputs_2d, name='mask_rcnn')
        """
        return model
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image, window, padding = utils.resize_image(image)            
            # Build image_meta
            image_meta = dg.compose_image_meta(image.shape, molded_image.shape, window,
    ...
    def test(self, images, labs, verbose = 1):
        assert self.mode == "inference", "Create model in inference mode."
        assert 1 == self.config.BATCH_SIZE, "Set BATCH_SIZE to 1."
        gt_bboxs, gt_class_ids, gt_masks = [], [], []
        ...
        for i in range(images.shape[0]):
            image, image_meta,\
            gt_class_id, gt_bbox,\
            gt_mask = dg.load_image_gt(images[i], labs[i], 
                                    self.config.NUM_CLASSES, 
                                    list(range(1, self.config.NUM_CLASSES)), 
                                    config,
                                    mode = self.mode)
...
def data_generator(imgs, masks, config, num_classes, class_ids, shuffle=True, batch_size=5, detection_targets=False, mode='training'):
    ...
    image_index = -1
    image_ids = np.arange(imgs.shape[0])    
    error_count = 0
    ...
    while True:
        try:
            ...
            image, image_meta, gt_class_ids, gt_boxes, gt_masks = \
                dg.load_image_gt(imgs[image_id], masks[image_id], num_classes, class_ids, config, mode=mode)
            

            # RPN Targets
            rpn_match, rpn_bbox = dg.build_rpn_targets(image.shape, anchors,
