        ...
        if propertyCount:
            for i in range(propertyCount):
                ...
                if propertyType in PROPERTY_TYPE_DICT.keys():
                    ...
                    self.propertyDict[propertyName.decode("utf-8")] = propertyValue

                elif propertyType in [4, 5]:
                    propertyLength = struct.unpack('<i', self.f.read(4))[0]
                    if propertyType == 4:
                        fmt, fmt_length = '<d', 8
                    else:
                        fmt, fmt_length = '<i', 4

                    for _ in range(int(propertyLength/fmt_length)):
                    ...
                    print ("Format Type not valid! Name: %s, Type: %s" % (propertyName, propertyType))
    ...
    def lut_reader(self, w, h, lut):
        ...
        for count, b in enumerate(lut):
            if count>= complete_bytes:
                ...
                # byte
                print("count larger than complete bytes, partial bytes: " + str(partial_bytes))
